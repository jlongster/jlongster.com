 ...
 */
function getPathFromCircleGroup(
  groupOfCircles = [],
  sorter = SORT.vertically,
  handleSize,
  v,
) {
  groupOfCircles.forEach(circ => {
  ...
  const pathFromCircles = (c, cn, i, len, down) => {
    ...
    if (i === len - 1) {
      ...
      d += `A ${down ? c.r : c.r1}, ${down ? c.r : c.r1}, 0, 1, 0, ${
        c.p[down ? 3 : 0]
      } `;
    ...
    d += `C ${c.h[down ? 0 : 3]} ${c.h[down ? 2 : 1]} ${c.p[down ? 2 : 1]} `;
    d += `A ${down ? c.r : c.r1}, ${down ? c.r : c.r1}, 0, 0, 0, ${
      cn.p[down ? 0 : 3]
    } `;
  ...
  pointsAndHandles = pointsAndHandles.filter(
  ...
  pointsAndHandles.forEach((c, i) => {
    d += pathFromCircles(
      c,
      pointsAndHandles[i + 1],
      i,
      pointsAndHandles.length,
      true,
    );
  });
  pointsAndHandles.reverse().forEach((c, i) => {
    d += pathFromCircles(
      c,
      pointsAndHandles[i + 1],
      i,
...
export { SORT, getPathFromCircleGroup, curvesBetweenCircles };
