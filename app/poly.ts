...
export function nearlyEquals(a, b, t = 0.000001) {
...
function linesIntersect(a0: Point, a1: Point, b0: Point, b1: Point) {
  ...
  const pt = { x: a0.x + A * adx, y: a0.y + A * ady };
...
function eventCompare(
  ...
  return pointAboveOrOnLine(p12, p2isStart ? p21 : p22, p2isStart ? p22 : p21)
    ? 1
    : -1;
}

function eventAdd(
  eventRoot: LinkedList<Event>,
  ev: Node<Event>,
  otherPt: Point,
) {
  eventRoot.insertBefore(
    ev,
    here =>
      eventCompare(
        !!ev.isStart,
...
function addSegmentStart(
  eventRoot: LinkedList<Event>,
  seg: Segment,
  primary: boolean,
) {
  const evStart = LinkedList.node({
    isStart: true,
    pt: seg.start,
    seg,
    primary,
...
function calculate(eventRoot: LinkedList<Event>, selfIntersection: boolean) {
  ...
  while (eventRoot.head) {
    ...
    if (ev.isStart) {
      const surrounding = statusRoot.findTransition(
        here => statusCompare(ev, here.ev) > 0,
      );
      ...
      if (eve) {
        ...
        if (selfIntersection) {
          ...
          const toggle = !ev.seg.myFill.below
            ? true
      ...
      if (selfIntersection) {
        ...
        const inside = !below
          ? false
          : ev.primary === below.primary
          ? below.seg.otherFill.above
          : below.seg.myFill.above;
...
export const union = (p1: MultiPolygon, p2: MultiPolygon) =>
  operate(p1, p2, UNION);
export const intersect = (p1: MultiPolygon, p2: MultiPolygon) =>
  operate(p1, p2, INTERSECT);
export const difference = (p1: MultiPolygon, p2: MultiPolygon) =>
  operate(p1, p2, DIFFERENCE);
